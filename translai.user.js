...
class NameManager {
    static async init() {
        ...
        this.bannedNames = await GM.getValue(`bannedNames:${Novel.id}`) || [];
    }
    ...
    static async save() {
        ...
        await GM.setValue(`bannedNames:${Novel.id}`, this.bannedNames);
    }
    ...
    static async copyName() {
        ...
        if (this.getSubState(name)) {
            await GM.setClipboard(`${name.subTranslated}: ${name.subOriginal}\n${name.translated}: ${name.original}`, 'text');
        } else {
    ...
    static getSubState(name) {
        ...
        for (const globalName of globalNames) {
            if (globalName.original.includes(name.original) && globalName.translated.includes(name.translated)) {
                name.subOriginal = globalName.original;
                name.subTranslated = globalName.translated;
                return 1;
            }
        ...
        for (const checkedName of checkedNames) {
            if (checkedName.original.includes(name.original) && checkedName.translated.includes(name.translated)) {
                name.subOriginal = checkedName.original;
                name.subTranslated = checkedName.translated;
                return 3;
            }
...
if (url.includes('book')) {
    ...
    new Button('🆕', NameManager.addNewName.bind(NameManager));
